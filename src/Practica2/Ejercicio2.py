...
while count < len(x):
    palabra = x[count]
    maximoNuevo = 0
    while count < len(x) and palabra == x[count]:
        maximoNuevo += 1
        count += 1
    if maximoNuevo > maximoTotal:
        maximoTotal = maximoNuevo
        palabraMaximo = palabra

print("la palabra con mas ocurrencias es: ", palabraMaximo, " la cantidad de veces que aparecio es: ", maximoTotal )
